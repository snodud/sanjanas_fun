...
def bin_iteration():
    ...
    cont = 'place_holder'
    while cont != '':
        #y_add = int(input('What is distance from corner of aisle to first bin location? ')) 
        if x_y_static == 'y':
...
def aisle_info():
    ...
    global aisle, ext_ID, aisle_sections, aisle_description, static_coord
    cont = 'place_holder'
    #Aisle Information
    while cont != '':
        #aisle = input('What is the aisle identifier?  ')
...
def export_csv():
    ...
    if len(ext_ID_lst) == 1:
        ...
        new_filename = 'A' + str(ext_ID_lst[0]) + '_to_' + 'A' + str(ext_ID) + '.csv'
    if update_i != 0:
