...
#fixed static coord, added option for static z value, trouble shooting manual z entry, adding in a unique list, 
# added in grid_counter to have multiple grid sections
...
def bin_iteration():
    global total_length, uniq_counter, y_int, y_fin, y_static, extID_lst, extID_i, ext_ID, sc_i, grid_counter
    ...
    if grid == 'n':
        while cont != '':
            ...
            y_int *= 1000 
            print('')
            cont = input(color_gren + 'Confirm Coordinates - Press enter. Else - Press any letter.  ' + res)  

    else:
        if grid == 'y' and update_i == 0:
            while cont != '':
                print('') 
                print('%s%sEnter Varied Coordinate Info: %s' % (fg('dark_orange'), attr('bold'), attr('reset')))
                if x_y_static == 'y':
                    y_int = 1.0#float(input('What is y initial?  '))
                    y_fin = 10.0#float(input('What is y final?  '))
    ...
    if (uniq_section[uniq_counter] - 1) != var_counter and (grid_section[grid_counter] - 1) != var_counter:
    ...
    elif (uniq_section[uniq_counter] - 1) != var_counter and (grid_section[grid_counter] - 1) == var_counter:
        print('grid comp!!')
        #unq_lst_len.append(str(0))
        grid_i = int(v_initial[0])
        p_step = (int(var_step[0]))
        ext_ID = str(extID_lst[extID_i])
        ###add this to unique grid - see if this works 
        static_coord = str(round(float(sc_lst[sc_i])) * 1000)
        # static_coord = str(sc_lst[sc_i])
        # static_coord = str(round(float(static_coord) * 1000))
    ...
    for i in range(total_length):
        if x_y_static == 'y':
            POS_Y.append(str(round(y)))
        else:
            POS_X.append(str(round(y)))
        y_counter += 1
        if y_counter > y_static: 
            y += y_step
            y_counter = 1  
    if ((uniq_section[uniq_counter] - 1) == var_counter and len(uniq_section) > 1): uniq_counter +=1       
    if ((grid_section[grid_counter] - 1) == var_counter and len(grid_section) > 1): grid_counter +=1 
...
def unique_iteration():
    ...
    cont = 'place_holder'
    while cont != '':
        lst = 'A B'#input('Please input unique list with a space in between each item:  ')
        cont = input('Confirm Unique Section Info - Press enter. Else - Press any letter.  ')
    lst = lst.split()
    p_static = int(var_static[var_counter])  
    unq_lst_len.append(str(len(lst)))
...
'''This function is used to specify the unique component for the barcode'''
def unique_component():
    global uniq_lst
    uniq_lst = []
    cont = 'place_holder'
    while cont != '':
        for i in range(len(uniq_section)):
            #print(color_orc + 'Enter Info For Unique Var Section' + str(var_counter + 1) + res)
            ##add for what var section
            lst = input('Please input unique list with a space in between each item:  ')
            lst = lst.split()
            uniq_lst.append(lst)
        cont = color_gren + input('Confirm Unique Section Info - Press enter. Else - Press any letter.  ') + res
...
def grid_iteration():
    ...
    #Unique Grid Component Information
        ...
        #     BARCODE[zoo] += (str(grid_i).zfill(fill))
        #     zoo += 1
    if len(grid_section) > 1: grid_counter += 1
...
def aisle_info():
    ...
    global aisle, ext_ID, aisle_sections, aisle_description, static_coord, sc_lst, grid_section
    ...
    while cont != '':
        if grid == 'n':
            ...
            sc_lst = '2 5 10'#input('Please input ext ID list with a space in between each item:  ')
            sc_lst = sc_lst.split()
            ...
            grid_section = '1'#input('Please input var section number for each grid component with a space in between each item:  ')
            grid_section = grid_section.split()
            for i in range(0, len(grid_section)): 
                grid_section[i] = int(grid_section[i])
        print('')
...
def x_z_uniq_info():
    global x_y_static, z_change, z_value, unique, uniq_section, uniq_lst, z_int, z_fin, z_step, z_static, z_value
    print('')
    ...
    while cont != '':
        ...
        # else:
        #     z_value = 1
        unique = 'y'#input('Is there an unique component (y/n)?  ')
        if unique == 'y': 
            print('')
            print('%s%sEnter Unique Section Info: %s' % (fg('light_goldenrod_2b'), attr('bold'), attr('reset')))
            uniq_section = '2 3'#input('Please input unique var section list with a space in between each item:  ')
            uniq_section = uniq_section.split()
            for i in range(0, len(uniq_section)): 
                uniq_section[i] = int(uniq_section[i])
            unique_component()
...
def z_coordinate():
    ...
    if z_change == 'y':
        z_value = 1
        default_z = input('Default z value to 1 -  Press enter. Else - Press any letter.  ')
        if default_z != '':
            cont = 'place holder'
            while cont != '':
                z_value = int(input('Enter static z value.  '))
                cont = input(color_gren + 'Confirm static Z value Info - Press enter. Else - Press any letter.  ' + res)
    #Z Corresponds to Var Section - Or Input Manually
    else:
...
###########################################################################################################################
print('')
grid = input(bold + 'Are the aisles in a grid pattern?(y/n)  ' + res).lower()

#Code Block for Unique Aisles
if grid == 'n':
    #Lists for final csv 
...
#Code Block for Grid Aisles
    ...
    for m in range(1, total_aisles + 1):
        ...
        var_counter = 0; static_counter = 0; uniq_counter = 0; grid_counter = 0; barcode_int = ''
        grid_index += 1
